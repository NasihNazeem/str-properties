...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--hero-gradient', 
      `linear-gradient(135deg, hsl(${primaryHsl}) 0%, hsl(${primaryHsl.replace(/(\d+)%/, (match, p1) => `${Math.min(100, parseInt(p1) + 10)}%`)}) 50%, hsl(${secondaryHsl}) 100%)`
    );
    root.style.setProperty('--trust-gradient', 
      `linear-gradient(90deg, hsl(${secondaryHsl}) 0%, hsl(${secondaryHsl.replace(/(\d+)%/, (match, p1) => `${Math.min(100, parseInt(p1) + 5)}%`)}) 100%)`
    );
